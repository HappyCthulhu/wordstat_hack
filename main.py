...
def main_func():
    ...
    def search(stop_count, search_key):
        ...
        while stop_count > count_now:
            ...
            logger.debug(f'Разослано запросов: {count_now}')
        ...
        logger.debug(
            f'Работа успешно завершена. Осталось {len(request_list)} ключей запроса. '
            f'Результаты можете посмотреть в report.txt и final_report.txt')
        return email, count_now, start_time, end_time

    proxy_list = get_proxies_from_file()

    report_file = open('text_files\\report.txt', 'a', encoding='utf-8')

    with open('text_files\\emails.yml', 'r', encoding='utf-8') as emails_file:
        emails_list = yaml.load(emails_file, Loader=yaml.FullLoader)
    logger.debug('Распаковали email.yml')

    with open('text_files\\requests.yml', 'r', encoding='utf-8') as request_file:
    ...
    for i in range(len(request_list)):
        ...
        email, request_count, start_time, end_time = search(count, search_key)
        wasted_time = end_time - start_time
        final_report = open('text_files\\final_report.txt', 'a', encoding='utf-8-sig')
        final_report.write(
            f'\ndate,time:{datetime.now()}| wasted time:{wasted_time}| IP:{ip}| search key:{search_key}| '
            f'number of requests:{request_count}| email:{email}')
        final_report.close()
        driver.quit()
